...
def preprocessing(img_path): 
  ...
  # print("Platform: ", platform.system(),"Preprocessing image:", command)
...
def test_net(canvas_size, mag_ratio, net, image, text_threshold, link_threshold, low_text, cuda, poly, refine_net=None):
    ...
    img_resized, target_ratio, size_heatmap = imgproc.resize_aspect_ratio(image, canvas_size, interpolation=cv2.INTER_CUBIC, mag_ratio=mag_ratio)
...
import re
def x_y_re(listTest):
    process_test = listTest
    process_test = list(map(lambda x: x.replace('\n',''),process_test))
    process_test = list(map(lambda x: x.replace('\x0c',''),process_test))
    final_test = []
    for x in process_test:
        if len(x) < 7:
            continue
        else :
            final_test +=  [x]
    ...
    pattern_x_2 = r'\d\d\d\d\d\d\d\.\d\d\d'
    pattern_y_2 = r'\d\d\d\d\d\d\.\d\d\d'
    for x,y in zip(process_test,process_test[1:]):
        x_nor = re.fullmatch(pattern_x,x)
        y_nor = re.fullmatch(pattern_y,y)
        if x_nor != None and y_nor != None:
            x_y_loc += [(final_test.index(x_nor.group(0)),final_test.index(y_nor.group(0)))]
            good_x_y += [(x_nor.group(0),y_nor.group(0))]
        else: 
            continue
    if not good_x_y:
        for x,y in zip(process_test,process_test[1:]):
            x_nor = re.fullmatch(pattern_x_2,x)
            y_nor = re.fullmatch(pattern_y_2,y)
            if x_nor != None and y_nor != None:
                x_y_loc += [(final_test.index(x_nor.group(0)),final_test.index(y_nor.group(0)))]
                good_x_y += [(x_nor.group(0),y_nor.group(0))]
            else: 
                continue
    if not good_x_y:
      for x,y in zip(final_test[::-1],final_test[:-1]):
        x_nor = re.fullmatch(pattern_x,x)
        y_nor = re.fullmatch(pattern_y,y)
        if x_nor != None and y_nor != None:
            x_y_loc += [(final_test.index(x_nor.group(0)),final_test.index(y_nor.group(0)))]
    ...
    print(good_x_y)
    # print(len(good_x_y))   
    return good_x_y[len(good_x_y)-1] if good_x_y else (0,0)
